  ...
  class DTOSecFilingResultDto
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::DTOSecFilingResultDto` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::DTOSecFilingResultDto`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'document_description')
        self.document_description = attributes[:'document_description']
      end
    ...
    def hash
      [accession_number, form_type, filing_date, company_name, cik, document_filename, document_description, source_file].hash
    end
