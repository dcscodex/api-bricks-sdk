#' Create a new OHLCVTimeseriesPeriod
#'
#' @description
#' Represents a timeseries period used in exchange rate data.
#'
#' @docType class
#' @title OHLCVTimeseriesPeriod
#' @description OHLCVTimeseriesPeriod Class
...
#' @export
OHLCVTimeseriesPeriod <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new OHLCVTimeseriesPeriod class.
    #'
    ...
    #'
    #' @return OHLCVTimeseriesPeriod as a base R list.
    #' @examples
    #' # convert array of OHLCVTimeseriesPeriod (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert OHLCVTimeseriesPeriod to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      OHLCVTimeseriesPeriodObject <- list()
      if (!is.null(self$`period_id`)) {
        OHLCVTimeseriesPeriodObject[["period_id"]] <-
          self$`period_id`
      }
      if (!is.null(self$`length_seconds`)) {
        OHLCVTimeseriesPeriodObject[["length_seconds"]] <-
          self$`length_seconds`
      }
      if (!is.null(self$`length_months`)) {
        OHLCVTimeseriesPeriodObject[["length_months"]] <-
          self$`length_months`
      }
      if (!is.null(self$`unit_count`)) {
        OHLCVTimeseriesPeriodObject[["unit_count"]] <-
          self$`unit_count`
      }
      if (!is.null(self$`unit_name`)) {
        OHLCVTimeseriesPeriodObject[["unit_name"]] <-
          self$`unit_name`
      }
      if (!is.null(self$`display_name`)) {
        OHLCVTimeseriesPeriodObject[["display_name"]] <-
          self$`display_name`
      }
      return(OHLCVTimeseriesPeriodObject)
    },

    #' @description
    #' Deserialize JSON string into an instance of OHLCVTimeseriesPeriod
    #'
    #' @param input_json the JSON input
    #' @return the instance of OHLCVTimeseriesPeriod
    ...
    #' @return OHLCVTimeseriesPeriod in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to OHLCVTimeseriesPeriod and throw an exception if invalid
    #'
    ...
    #'
    #' @return String representation of OHLCVTimeseriesPeriod
    toString = function() {
      self$toJSONString()
    },
...
# OHLCVTimeseriesPeriod$unlock()
#
## Below is an example to define the print function
# OHLCVTimeseriesPeriod$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# OHLCVTimeseriesPeriod$lock()
