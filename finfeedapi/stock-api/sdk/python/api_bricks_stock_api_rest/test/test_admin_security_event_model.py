# coding: utf-8

"""
    REST API

    No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

    The version of the OpenAPI document: v1
    Contact: support@apibricks.io
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from api_bricks_stock_api_rest.models.admin_security_event_model import AdminSecurityEventModel

class TestAdminSecurityEventModel(unittest.TestCase):
    """AdminSecurityEventModel unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> AdminSecurityEventModel:
        """Test AdminSecurityEventModel
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `AdminSecurityEventModel`
        """
        model = AdminSecurityEventModel()
        if include_optional:
            return AdminSecurityEventModel(
                symbol = '',
                timestamp_nanos = 56,
                timestamp = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'),
                security_event = 56,
                security_event_code = '',
                security_event_text = '',
                is_opening_process_complete = True,
                is_closing_process_complete = True
            )
        else:
            return AdminSecurityEventModel(
        )
        """

    def testAdminSecurityEventModel(self):
        """Test AdminSecurityEventModel"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
